 ...
 * @brief CAN ID, enum, payload pack/unpack helper
 * @{
 */
...
/**
 * @brief 
 * CAN identifier used for command frames sent from the driver ECU to the door ECU.
 * 
 * 운전석 ECU에서 도어 ECU로 보내는 command 프레임에 사용되는 CAN 식별자입니다.
 */
#define DOOR_CAN_ID_COMMAND   (0x100U)

/**
 * @brief 
 * CAN identifier used for status frames sent from the door ECU to the driver ECU.
 * 
 * 도어 ECU에서 운전석 ECU로 보내는 status 프레임에 사용되는 CAN 식별자입니다.
 */
...
/**
 * @brief 
 * Door command field values carried in a command payload.
 * 
...
/**
 * @brief 
 * Ramp command field values carried in a command payload.
 * 
...
/**
 * @brief 
 * Door state field values carried in a status payload.
 * 
...
/**
 * @brief 
 * Ramp state field values carried in a status payload.
 * 
...
/**
 * @brief 
 * Pack door/ramp command fields into one command payload byte.
 * 
 * door/ramp command 필드들을 하나의 command payload 바이트로 패킹합니다.
 *
 * @param door_cmd 
 * Door command field value.
 * door command 필드 값입니다.
 *
 * @param ramp_cmd 
 * Ramp command field value.
 * ramp command 필드 값입니다.
 *
 * @param emergency_stop 
 * Emergency-stop flag.
 * 비상 정지 플래그입니다.
 *
 * @param reset_fault 
 ...
 *
 * @return 
 * Packed command payload byte.
 * 패킹된 command payload 바이트를 반환합니다.
 */
                                         ...
                                         bool reset_fault)
{
    return (uint8_t)(
        ((door_cmd & 0x3u) << 0) |
        ((ramp_cmd & 0x3u) << 2) |
        ((emergency_stop ? 1u : 0u) << 4) |
        ((reset_fault ? 1u : 0u) << 5));
}

/**
 * @brief 
 * Pack door/ramp status fields into one status payload byte.
 * 
 * door/ramp status 필드들을 하나의 status payload 바이트로 패킹합니다.
 *
 * @param pinch 
 * Pinch-detection flag.
 * 끼임 감지 플래그입니다.
 *
 * @param door_state 
 * Door state field value.
 * door state 필드 값입니다.
 *
 * @param ramp_state 
 * Ramp state field value.
 * ramp state 필드 값입니다.
 *
 * @param fault_present 
 * Fault-present flag.
 ...
 *
 * @return 
 * Packed status payload byte.
 * 패킹된 status payload 바이트를 반환합니다.
 */
...
/**
 * @brief 
 * Extract the door command field from a command payload byte.
 * 
 * command payload 바이트에서 door command 필드를 추출합니다.
 *
 * @param payload 
 * Packed command payload byte.
 * 패킹된 command payload 바이트입니다.
 *
 * @return 
 * Door command field value.
 * door command 필드 값을 반환합니다.
 */
...
/**
 * @brief 
 * Extract the ramp command field from a command payload byte.
 * 
...
/**
 * @brief 
 * Extract the emergency-stop flag from a command payload byte.
 * 
 ...
 *
 * @return 
 * true if emergency stop is requested, otherwise false.
...
/**
 * @brief 
 * Extract the fault-reset flag from a command payload byte.
 * 
 ...
 *
 * @return 
 * true if fault reset is requested, otherwise false.
...
/**
 * @brief 
 * Extract the pinch-detection flag from a status payload byte.
 * 
 * status payload 바이트에서 끼임 감지 플래그를 추출합니다.
 *
 * @param payload 
 * Packed status payload byte.
 * 패킹된 status payload 바이트입니다.
 *
 * @return 
 * true if pinch is reported, otherwise false.
...
/**
 * @brief 
 * Extract the door state field from a status payload byte.
 * 
...
/**
 * @brief 
 * Extract the ramp state field from a status payload byte.
 * 
...
/**
 * @brief 
 * Extract the fault-present flag from a status payload byte.
 * 
 ...
 *
 * @return 
 * true if a fault is reported, otherwise false.
...
/**
 * @brief 
 * Extract the alive-toggle flag from a status payload byte.
 * 
 ...
 *
 * @return 
 * true if the alive-toggle bit is set, otherwise false.
